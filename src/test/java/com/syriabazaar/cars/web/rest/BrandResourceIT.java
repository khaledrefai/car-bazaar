...
import static com.syriabazaar.cars.domain.BrandAsserts.*;
...
import com.syriabazaar.cars.domain.Brand;
import com.syriabazaar.cars.repository.BrandRepository;
...
class BrandResourceIT {
    ...
    void createBrand() throws Exception {
        ...
        var returnedBrand = om.readValue(
            restBrandMockMvc
                .perform(post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(brand)))
    ...
    void putWithMissingIdPathParamBrand() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restBrandMockMvc
            .perform(put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(brand)))
    ...
    void patchWithMissingIdPathParamBrand() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restBrandMockMvc
            .perform(patch(ENTITY_API_URL).with(csrf()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(brand)))
            .andExpect(status().isMethodNotAllowed());
