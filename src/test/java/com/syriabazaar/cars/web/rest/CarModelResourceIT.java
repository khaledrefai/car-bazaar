...
import static com.syriabazaar.cars.domain.CarModelAsserts.*;
...
import com.syriabazaar.cars.repository.CarModelRepository;
...
class CarModelResourceIT {
    ...
    void createCarModel() throws Exception {
        ...
        var returnedCarModel = om.readValue(
            restCarModelMockMvc
                .perform(post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(carModel)))
    ...
    void putNonExistingCarModel() throws Exception {
        ...
        restCarModelMockMvc
            .perform(
                put(ENTITY_API_URL_ID, carModel.getId())
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
    ...
    void putWithMissingIdPathParamCarModel() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCarModelMockMvc
            .perform(put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(carModel)))
    ...
    void patchWithMissingIdPathParamCarModel() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCarModelMockMvc
            .perform(patch(ENTITY_API_URL).with(csrf()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(carModel)))
            .andExpect(status().isMethodNotAllowed());
    ...
    void deleteCarModel() throws Exception {
        ...
        // Delete the carModel
        restCarModelMockMvc
            .perform(delete(ENTITY_API_URL_ID, carModel.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
