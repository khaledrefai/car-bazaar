...
import static com.syriabazaar.cars.domain.CarTypeAsserts.*;
...
import com.syriabazaar.cars.repository.CarTypeRepository;
...
class CarTypeResourceIT {
    ...
    void createCarType() throws Exception {
        ...
        var returnedCarType = om.readValue(
            restCarTypeMockMvc
                .perform(post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(carType)))
    ...
    void putNonExistingCarType() throws Exception {
        ...
        restCarTypeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, carType.getId())
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
    ...
    void putWithMissingIdPathParamCarType() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCarTypeMockMvc
            .perform(put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(carType)))
    ...
    void patchNonExistingCarType() throws Exception {
        ...
        restCarTypeMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, carType.getId())
                    .with(csrf())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(carType))
            )
    ...
    void patchWithMissingIdPathParamCarType() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCarTypeMockMvc
            .perform(patch(ENTITY_API_URL).with(csrf()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(carType)))
            .andExpect(status().isMethodNotAllowed());
    ...
    void deleteCarType() throws Exception {
        ...
        // Delete the carType
        restCarTypeMockMvc
            .perform(delete(ENTITY_API_URL_ID, carType.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
