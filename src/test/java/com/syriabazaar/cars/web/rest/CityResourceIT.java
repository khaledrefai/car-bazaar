...
import static com.syriabazaar.cars.domain.CityAsserts.*;
...
import com.syriabazaar.cars.repository.CityRepository;
...
class CityResourceIT {
    ...
    void createCity() throws Exception {
        ...
        var returnedCity = om.readValue(
            restCityMockMvc
                .perform(post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(city)))
    ...
    void putWithMissingIdPathParamCity() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCityMockMvc
            .perform(put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(city)))
    ...
    void patchWithMissingIdPathParamCity() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCityMockMvc
            .perform(patch(ENTITY_API_URL).with(csrf()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(city)))
            .andExpect(status().isMethodNotAllowed());
