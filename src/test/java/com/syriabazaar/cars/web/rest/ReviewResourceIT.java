...
import static com.syriabazaar.cars.domain.ReviewAsserts.*;
...
class ReviewResourceIT {
    ...
    void createReview() throws Exception {
        ...
        var returnedReview = om.readValue(
            restReviewMockMvc
                .perform(post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(review)))
    ...
    void putWithMissingIdPathParamReview() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restReviewMockMvc
            .perform(put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(review)))
    ...
    void partialUpdateReviewWithPatch() throws Exception {
        ...
        partialUpdatedReview.revVal(UPDATED_REV_VAL);
    ...
    void patchWithMissingIdPathParamReview() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restReviewMockMvc
            .perform(patch(ENTITY_API_URL).with(csrf()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(review)))
            .andExpect(status().isMethodNotAllowed());
