...
import static com.syriabazaar.cars.domain.SellerAsserts.*;
import static com.syriabazaar.cars.web.rest.TestUtil.createUpdateProxyForBean;
...
import com.syriabazaar.cars.IntegrationTest;
import com.syriabazaar.cars.domain.Seller;
import com.syriabazaar.cars.repository.SellerRepository;
...
class SellerResourceIT {
    ...
    void createSeller() throws Exception {
        ...
        var returnedSeller = om.readValue(
            restSellerMockMvc
                .perform(post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(seller)))
    ...
    void putWithMissingIdPathParamSeller() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSellerMockMvc
            .perform(put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(seller)))
    ...
    void partialUpdateSellerWithPatch() throws Exception {
        ...
        partialUpdatedSeller.address(UPDATED_ADDRESS).whatsApp(UPDATED_WHATS_APP).insta(UPDATED_INSTA);
    ...
    void patchWithMissingIdPathParamSeller() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restSellerMockMvc
            .perform(patch(ENTITY_API_URL).with(csrf()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(seller)))
            .andExpect(status().isMethodNotAllowed());
    ...
    void deleteSeller() throws Exception {
        ...
        // Delete the seller
        restSellerMockMvc
            .perform(delete(ENTITY_API_URL_ID, seller.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
